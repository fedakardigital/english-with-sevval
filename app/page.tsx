...
const Navbar = ({ onHakkimdaClick, onBlogClick, onDerslerClick, onKurslarClick, onMailClick }: { onHakkimdaClick: () => void, onBlogClick: () => void, onDerslerClick: () => void, onKurslarClick: () => void, onMailClick: () => void }) => {
  ...
  const [showIletisimBilgi, setShowIletisimBilgi] = useState(false); // Yeni state
...
const Hero = ({ onDerslerClick }: { onDerslerClick: () => void }) => (
...
<Navbar
  ...
  onMailClick={() => setShowMailForm(true)} // İşte eksik olan parça bu Başkan!

/>

      <Hero onDerslerClick={() => setShowDersler(true)} />
